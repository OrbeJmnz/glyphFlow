...
describe('analizarIcono', () => {
  it('duración = delay + duration del track más largo', () => {
    const r = analizarIcono(
      'x',
      def({
        default: {
          root: { keyframes: [{ transform: 'scale(1)' }], options: { duration: 300 } },
          shapes: {
            0: { keyframes: [{ transform: 'scale(1)' }], options: { duration: 500, delay: 200 } },
          },
        },
      }),
    );
  ...
  it('rotación máxima toma el valor absoluto y el más grande de todos los keyframes', () => {
    const r = analizarIcono(
      ...
      def({
        default: {
          root: {
            keyframes: [{ transform: 'rotate(15deg)' }, { transform: 'rotate(-40deg)' }],
  ...
  it('animar `d` se marca como caro; transform/opacity no', () => {
    const barato = analizarIcono(
      'x',
      def({
        default: {
          root: {
            keyframes: [{ transform: 'scale(1)', opacity: '1' }],
            options: { duration: 300 },
          },
        },
      }),
    ...
    const r = analizarIcono(
      ...
      def({
        default: {
          root: {
            keyframes: [
              { offset: 0, easing: 'ease', transform: 'scale(1)' },
              { transform: 'scale(1.1)' },
            ],
            options: { duration: 300 },
          },
  ...
  it('analiza TODAS las variantes de la coreografía, no solo default', () => {
    const r = analizarIcono(
      ...
      def({
        ...
        rotate: {
          root: { keyframes: [{ transform: 'rotate(90deg)' }], options: { duration: 200 } },
        },
      }),
    );
