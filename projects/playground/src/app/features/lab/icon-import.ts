 ...
 */
/** Lo que el importador entiende: o el icono, o la clave del error, o nada si no hay texto. */
export type ResultadoImportacion =
  { def: AnimatedIconDef; nombre: string | null } | { errorKey: string } | null;

/**
 * Valida y convierte el JSON pegado. Función PURA y exportada a propósito, no un método del
 * componente: es el contrato de entrada del sitio, y quien tiene que poder comprobarlo sin montar
 * Angular es el test del viaje de ida y vuelta — el que afirma que lo que EXPORTA el editor es
 * exactamente lo que este importador acepta. Con la validación encerrada en un `computed`, ese
 * test tendría que instanciar dos componentes para comparar dos strings.
 *
 * Los mensajes viajan como CLAVE, no como texto: aquí no se sabe en qué idioma está el usuario, y
 * traducir sería imperativo y síncrono. El template resuelve la clave con el pipe.
 */
export function analizarImportacion(crudo: string): ResultadoImportacion {
...
export class IconImport {
   ...
   */
  /** El mismo análisis que usa el test del viaje redondo, atado a lo que se escribe en el área. */
  protected readonly resultado = computed<ResultadoImportacion>(() =>
    analizarImportacion(this.entrada()),
  );
