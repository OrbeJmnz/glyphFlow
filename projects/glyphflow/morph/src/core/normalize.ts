/* MIT License — Copyright (c) 2026 Guillermo
   https://github.com/guillermolg00/morphicons — see LICENSE-morphicons for the
   full permission notice.

   VENDORIZADO de morphicons v1.7.0 (src/core/normalize.ts).
   Cualquier cambio aquí es una divergencia deliberada del upstream: documenta
   el porqué en el commit.

   DIVERGENCIA (única, 3 líneas, marcadas abajo con [glyphflow]): `attrs.d` y
   `attrs.points` pasaron a `attrs['d']` / `attrs['points']`. Este workspace
   compila con `noPropertyAccessFromIndexSignature`, que upstream no usa, y
   `IconNodeAttrs` es un index signature. Es puramente sintáctico: mismo acceso,
   misma semántica, cero matemática tocada. Al sincronizar con una versión nueva
   de morphicons hay que volver a aplicarlo. */
...
export function iconToCubics(input: IconInput): CubicPath[] {
  ...
  for (const [tag, attrs] of input) {
    switch (tag) {
      case "path":
        for (const s of parsePath(String(attrs["d"] ?? ""))) push(lowerSubpath(s)); // [glyphflow]
        break;
      case "line": {
      ...
      case "rect":
        ...
        break;
      case "polyline":
        push(polyPath(parsePoints(attrs["points"]), false)); // [glyphflow]
        break;
      case "polygon":
        push(polyPath(parsePoints(attrs["points"]), true)); // [glyphflow]
        break;
      default:
